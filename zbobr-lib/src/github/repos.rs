...
impl Zbobr {
    ...
    /// Clone a repo for read-only investigation and checkout specific branch (no fork).
    pub(crate) async fn clone_readonly(
        ...
        if !work_dir.exists() {
            ...
            // Use `git clone` instead of `gh repo clone` for read-only clones
            let clone_url = format!("https://github.com/{}.git", target_repo);
            let status = tokio::process::Command::new("git")
    ...
    pub(crate) async fn push_and_create_pr(
        ...
        // Create PR using the GitHub API via octocrab
    ...
    /// - "owner/repo#123"
    pub(crate) async fn parse_pr_to_repo_branch(
        &self,
        pr_ref: &str,
        ...
        // Use GitHub API via octocrab to get PR details (head ref)
        #[derive(serde::Deserialize)]
        struct PrHead {
            #[serde(rename = "ref")]
            ref_name: String,
