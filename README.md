...
- **Resolution Order**:
   ...
   4. `copilot_github_token` in the optional `[executor.copilot]` section of `zbobr.toml`
- **Config File**: `executor.copilot.copilot_github_token` in `zbobr.toml` (optional; configure via env var if not using TOML)
...
- Copilot token:
   ...
   - TOML field: `copilot_github_token` under optional `[executor.copilot]` section in `zbobr.toml` (can be omitted if using environment variables instead).
