...
- test_planner
  - ✅ Test plan ready: 3 additional tests needed for review fix gaps — priority inheritance from parent, validate rejects unknown executor, build_executor rejects unknown executor [ctx_rec_23]
    - [ ] Test priority inheritance from parent in `resolve_providers()` (zbobr-api/src/config.rs) [ctx_rec_20]
    - [ ] Test `validate()` rejects unknown executor names (zbobr-api/src/config.rs) [ctx_rec_21]
    - [ ] Test `build_executor()` returns error for unknown executor (zbobr-dispatcher/src/lib.rs) [ctx_rec_22]
