...
- test_planner
  - ✅ Test plan ready: 4 groups of tests needed — resolve_providers (5 tests), resolve_tool_name (4 tests), validate (4 tests), select_provider (7 tests) [ctx_rec_14]
    - [ ] Unit tests for `select_provider()` in zbobr-dispatcher/src/lib.rs — priority selection, round-robin, excluded provider skip, all-excluded error, unknown tool error [ctx_rec_10]
    - [ ] Unit tests for `resolve_providers()` in zbobr-api/src/config.rs — basic resolution, single/multi-level inheritance, circular reference detection, field override precedence [ctx_rec_11]
    - [ ] Unit tests for `validate()` in zbobr-api/src/config.rs — valid config, unknown parent, missing executor+parent, unknown provider ref [ctx_rec_12]
    - [ ] Unit tests for `resolve_tool_name()` in zbobr-api/src/config.rs — stage precedence, role fallback, global fallback [ctx_rec_13]
