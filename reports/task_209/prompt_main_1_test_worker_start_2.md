...
- test_planner
  - ✅ Test plan ready: 3 groups of tests needed — Model::try_new validation (5 tests), validate() global tool check (2 tests), validate_workflow_refs() (4 tests) [ctx_rec_34]
    - [ ] Unit tests for `validate()` global tool check in zbobr-api/src/config.rs — rejects unknown global tool, passes when tools map empty, passes when tool exists [ctx_rec_31]
    - [ ] Unit tests for `validate_workflow_refs()` in zbobr-api/src/config.rs — rejects unknown role tool, rejects unknown stage tool, passes valid refs, passes when no tool refs [ctx_rec_32]
    - [ ] Unit tests for `Model::try_new()` in zbobr-api/src/task.rs — valid name, rejects spaces/tabs/newlines, empty string OK, FromStr error propagation [ctx_rec_33]
