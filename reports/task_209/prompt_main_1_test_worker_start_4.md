...
- test_planner
  - ✅ Test plan ready: 2 groups of tests needed — detect_quota_failure (7 tests) and stage-title malformed model rejection (2 tests) [ctx_rec_49]
    - [ ] Unit tests for `detect_quota_failure()` in zbobr-api/src/tool_executor.rs — matches each pattern, negative cases, case-insensitivity [ctx_rec_47]
    - [ ] Unit test for stage-title parsing rejecting malformed model token in zbobr-api/src/context/stage_title.rs [ctx_rec_48]
