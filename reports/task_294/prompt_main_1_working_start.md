...
- planning
  - ✅ Plan approved and checklist items created for `linter_worker` stage implementati [ctx_rec_8]
    - [ ] Update `linting` stage: change `on_failure` from `working` to `linter_worker` [ctx_rec_2]
    - [ ] Add `linter_worker` stage to `main_stages` between `linting` and `testing` [ctx_rec_3]
    - [ ] Add `linter_worker` role definition in `init.rs` [ctx_rec_4]
    - [ ] Update `LINTER_PROMPT` to be check-only (remove auto-fix logic) [ctx_rec_5]
    - [ ] Add `LINTER_WORKER_PROMPT` constant and register it in `PROMPT_FILES` [ctx_rec_6]
    - [ ] Build and verify compilation succeeds [ctx_rec_7]
