...
- planning
  - ✅ Plan approved — 6 checklist items created following the `instance` injection pat [ctx_rec_8]
    - [ ] Add `timezone` field to GitHub backend config [ctx_rec_2]
    - [ ] Add `timezone` field to FS backend config [ctx_rec_3]
    - [ ] Apply timezone conversion in GitHub backend's `get_task_comments_internal()` [ctx_rec_4]
    - [ ] Apply timezone conversion in FS backend's `read_comments_structured()` [ctx_rec_5]
    - [ ] Inject timezone from dispatcher config at backend construction site [ctx_rec_6]
    - [ ] Update test construction sites to set `timezone` field [ctx_rec_7]
