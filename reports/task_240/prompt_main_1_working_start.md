...
- planning
  - ✅ Plan approved and checklist created. Approach: extract shared ready-task selection from loop, add TaskListEntry for compact display, update task list/show with --json and --select flags. [ctx_rec_5]
    - [ ] Create TaskListEntry struct and extract shared ready-task selection function [ctx_rec_2]
    - [ ] Update `task list` command: compact format, --json flag, --select flag [ctx_rec_3]
    - [ ] Add --json flag to `task show` for full Task JSON output [ctx_rec_4]
