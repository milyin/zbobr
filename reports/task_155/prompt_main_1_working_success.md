...
Plan ready. Two features: (1) sort tasks by stage_count descending so highest-progress tasks are processed first, (2) add max_task_stage_count config (default 20) propagated to per-task max_stage_count field — auto-pauses tasks when limit reached. Follows existing patterns: confirm flag for pause mechanism, RoleSession field protection for agent safety, config_struct macro for config fields.

[report_main_1_planning_success.md](https://github.com/milyin/zbobr/blob/reports/reports/task_155/report_main_1_planning_success.md)
...
- [ ] [id: config-max-stage-count] Add `max_task_stage_count: u64` field to `ZbobrDispatcherConfig` in `zbobr-api/src/config.rs` with `#[arg(default_value = "20")]` and default `20`.
- [ ] [id: task-max-stage-count-field] Add `pub max_stage_count: u64` field with `#[serde(default)]` to `Task` struct in `zbobr-api/src/task.rs`.
- [ ] [id: protect-max-stage-count] Protect `max_stage_count` from agent modification in `RoleSession::modify_task` in `zbobr-dispatcher/src/task.rs` (save/restore alongside `stage_count`).
- [ ] [id: set-max-on-create] Set `task.max_stage_count` from `self.config.max_task_stage_count` in `create_task_with_confirm` in `zbobr-dispatcher/src/lib.rs`.
- [ ] [id: auto-pause-on-limit] After `increment_stage_count()` in `CliStageRunner::run()` (cli.rs:353) and `handle_call_stage` (cli.rs:595), check if `stage_count >= max_stage_count` (when `max_stage_count > 0`). If exceeded, set `task.pause = true`, log a warning, and return early.
- [ ] [id: sort-by-stage-count] After collecting `all_tasks` in the manager loop (cli.rs:923), sort by `stage_count` descending: `all_tasks.sort_by(|a, b| b.stage_count.cmp(&a.stage_count));` so tasks closest to completion are processed first.
- [ ] [id: verify-build-test] Run `cargo build` and `cargo test` to verify compilation and existing tests pass.