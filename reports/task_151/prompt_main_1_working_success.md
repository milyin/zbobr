...
Plan ready: change `TaskBackend::setup` trait to accept signal labels list, compute labels from workflow config in dispatcher, sync labels (delete obsolete + create missing) in GitHub backend setup. Analog: flag label setup pattern in github.rs:511-530.

[report_main_1_planning_success.md](https://github.com/milyin/zbobr/blob/reports/reports/task_151/report_main_1_planning_success.md)
...
- [ ] [id: change-setup-trait-signature] Change `TaskBackend::setup` signature in `zbobr-api/src/backend.rs` to accept `signal_labels: &[String]` parameter
- [ ] [id: add-delete-label-method] Add `delete_label` method to `ZbobrTaskBackendGithubImpl` in `zbobr-task-backend-github/src/github.rs` (near existing `create_label`/`update_label` methods), using `retry_github` and `DELETE /repos/{owner}/{repo}/labels/{name}`
- [ ] [id: implement-signal-sync-in-github-setup] Implement signal label sync in GitHub backend's `setup()` method (`github.rs:501`): compare existing `signal:*` labels with required ones, delete obsolete, create missing, update if force. Use `SIGNAL_LABEL_COLOR` constant. Follow the flag label pattern at lines 511-530 as analog.
- [ ] [id: compute-signal-labels-in-dispatcher] Compute required signal labels in `setup_repository()` in `zbobr-dispatcher/src/lib.rs:182`: derive `signal:go_{stage}` for all stages, `signal:call_{pipeline}` for all pipelines, plus `signal:return` and `signal:return_failure`. Use `self.workflow.config().pipeline_names()` and `all_stages()`. Pass to `task_backend.setup(force, &signal_labels)`.
- [ ] [id: update-all-setup-impls] Update all other `TaskBackend::setup` implementations to match new signature: `DummyBackend` in `zbobr-dispatcher/src/backend.rs:27`, test backend in `zbobr-dispatcher/src/task.rs:888`, FS backend in `zbobr-task-backend-fs/src/fs.rs:554,668`, outer GitHub impl in `github.rs:1252`
- [ ] [id: build-and-test] Run `cargo build` and `cargo test` to verify compilation and no regressions