...
Plan ready: Fix two review issues — add `ensure_label_exists` helper for dynamic label creation in `apply_state_change()`, and fix `state:pending` color from lavender to gray.

[report_main_1_planning_success_1.md](https://github.com/milyin/zbobr/blob/reports/reports/task_158/report_main_1_planning_success_1.md)
...
- [ ] [id: ensure-label-helper] Add `ensure_label_exists(&self, name, color, description)` helper method in `github.rs` after `create_label()` (~line 472). It should call `create_label()` and ignore 422/"already_exists" errors (label already exists). This follows the pattern used in `setup()` but is callable at runtime for dynamic labels.
- [ ] [id: dynamic-label-creation] In `apply_state_change()` (github.rs ~line 359), BEFORE calling `add_labels`, iterate over `new_labels` and call `self.ensure_label_exists(label, Self::state_label_color(label), &format!("State: {}", label)).await?;` for each label. This ensures dynamically-generated labels like `pipeline:custom` and `stage:custom` exist in the repo with correct colors before being assigned to issues.
- [ ] [id: fix-pending-color] In `state_label_color()` (github.rs line 287), change `state:pending` color from `d4c5f9` (lavender/purple) to `d3d3d3` (gray) to match the task requirement "pending is gray".
- [ ] [id: verify-tests] Run `cargo test` and `cargo clippy` in `zbobr-task-backend-github` to verify all tests pass and no warnings are introduced.