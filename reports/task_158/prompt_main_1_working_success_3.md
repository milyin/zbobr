...
Plan ready: eliminate all hardcoded label literals in github.rs by introducing prefix constants and using existing State::LABEL_*/Pipeline::MAIN/MERGE constants from task.rs, rewrite setup() to generate labels programmatically, and add type-derivation guidelines to worker/reviewer prompts in init.rs.

[report_main_1_planning_success_3.md](https://github.com/milyin/zbobr/blob/reports/reports/task_158/report_main_1_planning_success_3.md)
...
- [ ] [id: add-prefix-constants-rewrite-functions] Add `STATE_PREFIX`, `PIPELINE_PREFIX`, `STAGE_PREFIX` constants to `impl ZbobrTaskBackendGithubImpl` in `github.rs`. Rewrite `state_to_labels()`, `labels_to_state()`, `state_label_color()`, `apply_state_change()` to use these prefix constants combined with `State::LABEL_*` constants (from `zbobr-api/src/task.rs`) instead of hardcoded string literals. In `labels_to_state()`, match on `State::LABEL_DONE` etc. instead of `"done"`. In `state_label_color()`, strip the prefix first, then match on `State::LABEL_*`. In `apply_state_change()`, use `Self::STATE_PREFIX`/`PIPELINE_PREFIX`/`STAGE_PREFIX` for `starts_with` checks.
- [ ] [id: rewrite-setup-programmatic-labels] Rewrite the `setup()` function in `github.rs` to generate the state labels list programmatically instead of the hardcoded array at lines 652-660. Iterate `State::ALL_LABEL_NAMES` with `STATE_PREFIX` for state labels, and use `Pipeline::MAIN`/`Pipeline::MERGE` with `PIPELINE_PREFIX` for pipeline labels. No hardcoded label strings should remain.
...
- [ ] [id: verify-no-hardcoded-literals] Verify: run `cargo check` and `cargo test`. Grep github.rs for any remaining hardcoded label literals ("state:done", "state:pause", "state:ready", "state:pending", "state:running", "pipeline:main", "pipeline:merge"). Inline `"state:"`, `"pipeline:"`, `"stage:"` should only appear as constant definitions, not as scattered literals.