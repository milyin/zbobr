...
Plan ready: 3 checklist items to replace hardcoded label literals with type-derived constants in task.rs and github.rs, plus update worker/reviewer prompts in init.rs. Follows Pipeline::MAIN/MERGE + as_str() as the analog pattern.

[report_main_1_planning_success_2.md](https://github.com/milyin/zbobr/blob/reports/reports/task_158/report_main_1_planning_success_2.md)
...
- [ ] [id: add-label-constants-to-state] Add label name constants and methods to `State` in `zbobr-api/src/task.rs`: add `LABEL_DONE/PAUSE/READY/PENDING/RUNNING` constants (lowercase), `ALL_LABEL_NAMES` array, and `pub fn label_name(&self) -> Option<&str>` method. Follow the same pattern as `Pipeline::MAIN`/`MERGE` + `as_str()`.
