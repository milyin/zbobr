...
# Current task: systematize and reduce amount of labels

# Task description

The constant labels updading adds too much noice to comments feed. The task is to reduce label's usage and make them namespaced. Things to do:
- rename label `zbobr:<instance>` to `zb:node:<instance>`
- place state as the explicit field in the parameters section instead of calculating it from labels
- establish the following relations between labels and stage:
  - as before, process only the tasks assigned to node with `zb:node:<instance>` label
  - first process labels: 
    - no additional labels, state is not set -> set state to Ready 
    - label `zb:pause` is set, state is not `Pause` -> push current (state,pipeline), set state to Pause
  - then process state 
    - state is `Pause` -> set `zb:pause` label if not set
    - state is `Done` -> set `zb:done` label, remove `zb:pause`, `zb:assigned` labels
    - other state -> remove `zb:pause` label, set `zb:assigned` label if not set

The expected workflow is:
- empty new task just assigned to zb:node:<instance> gets state `Ready` and automatically gets label `state:assigned`
- task in progress (no matter pending or running) is kept marked as `state:assgined` without label changes on status change
- task set to `Pause` state by zbobr immediately gets `zb:pause`. This do not replace `zb:assigned` - labels can coexist
- task already in the `Pause` state but without `zb:pause` label (i.e. unpaused by user) switched to `Ready` state
- task in any state with `zb:pause` label by user gets Pause state and current state is pushed - i.e. user can set process to pause from github interface

Verify the if the expected workflow and label/state processing algorithm matches, report flaws and edge cases in it if any

Existing labels `state:pending`, `state:running`, and `state:ready` should be removed.

Do not keep any backward compatibility code.



# Destination branch: main

# Work branch: zbobr_fix-259-systematize-and-reduce-amount-of-labels
