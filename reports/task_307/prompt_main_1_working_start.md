...
- planning
  - ✅ Plan approved and checklist created for adding `--force` flag to `init` command [ctx_rec_6]
    - [ ] Add `force` field to `Init` variant in `commands.rs` [ctx_rec_2]
    - [ ] Pass `force` flag through `main.rs` to `init_workspace()` [ctx_rec_3]
    - [ ] Update `init.rs`: accept `force` param, change `write_or_new` behavior [ctx_rec_4]
    - [ ] Build and test [ctx_rec_5]
