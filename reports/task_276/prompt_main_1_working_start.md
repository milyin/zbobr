...
- planning
  - ✅ Plan approved and checklist created with 6 implementation steps [ctx_rec_8]
    - [ ] Update `validate_workflow_refs_passes_no_tool_refs` test to expect failure for tool-less role [ctx_rec_2]
    - [ ] Verify all tests compile and pass [ctx_rec_3]
    - [ ] Add validation in `validate_workflow_refs` requiring every role to have a `tool` [ctx_rec_4]
    - [ ] Fix global-tool validation tests that reference removed `config.tool` field [ctx_rec_5]
    - [ ] Fix `resolve_tool_name` tests that reference removed global `tool` field [ctx_rec_6]
    - [ ] Fix integration tests in `zbobr-dispatcher` referencing removed global `tool` field [ctx_rec_7]
