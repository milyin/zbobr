...
fn branch_isolation_instruction() -> String {
    let get_dest_branch = PlannerToolNames::GET_PARAM_DESTINATION_BRANCH;
    let get_work_branch = PlannerToolNames::GET_PARAM_WORK_BRANCH;
    format!(
        ...
        by the MCP tools `{get_dest_branch}`, `{get_work_branch}`. \
...
impl PromptBuilder for DefaultPromptBuilder {
    fn preparator_instructions(&self, _tools: &PreparatorToolNames) -> String {
        use PreparatorToolNames as T;
...
"#,
            REPORT_ERROR = T::REPORT_ERROR,
            ASK_USER = WorkerToolNames::ASK_USER,
            GET_PARAM_DESTINATION_REPOSITORY = T::GET_PARAM_DESTINATION_REPOSITORY,
            SET_PARAM_DESTINATION_REPOSITORY = T::SET_PARAM_DESTINATION_REPOSITORY,
            GET_PARAM_DESTINATION_BRANCH = T::GET_PARAM_DESTINATION_BRANCH,
            SET_PARAM_DESTINATION_BRANCH = T::SET_PARAM_DESTINATION_BRANCH,
            SET_PARAM_WORK_BRANCH_POSTFIX = T::SET_PARAM_WORK_BRANCH_POSTFIX,
            GET_PARAM_WORK_BRANCH = T::GET_PARAM_WORK_BRANCH,
            REPORT_RESULTS = T::REPORT_RESULTS,
        )
    }

    fn planner_instructions(&self, _tools: &PlannerToolNames) -> String {
        use PlannerToolNames as T;
...
"#,
            ASK_USER = T::ASK_USER,
            POST_PLAN = T::POST_PLAN,
            REPORT_ERROR = T::REPORT_ERROR,
            GET_HISTORY = T::GET_HISTORY,
            GET_PARAM_DESTINATION_BRANCH = T::GET_PARAM_DESTINATION_BRANCH,
            GET_PARAM_WORK_BRANCH = T::GET_PARAM_WORK_BRANCH,
            GET_CHECKLIST = T::GET_CHECKLIST,
            INSERT_CHECKLIST_ITEM = T::INSERT_CHECKLIST_ITEM,
            UPDATE_CHECKLIST_ITEM = T::UPDATE_CHECKLIST_ITEM,
            DELETE_CHECKLIST_ITEM = T::DELETE_CHECKLIST_ITEM,
        )
    }

    fn worker_instructions(&self, _tools: &WorkerToolNames) -> String {
        use WorkerToolNames as T;
        let branch_isolation = branch_isolation_instruction();
        format!(
            r#"# Worker Agent
...
11. Call `{REPORT_RESULTS}` to provide a brief and concise report of your work and finish the session. This report is critical context for further agent calls, so it MUST be compact."#,
            GET_CHECKLIST = T::GET_CHECKLIST,
            CHECK_CHECKLIST_ITEM = T::CHECK_CHECKLIST_ITEM,
            INSERT_CHECKLIST_ITEM = T::INSERT_CHECKLIST_ITEM,
            UPDATE_CHECKLIST_ITEM = T::UPDATE_CHECKLIST_ITEM,
            DELETE_CHECKLIST_ITEM = T::DELETE_CHECKLIST_ITEM,
            GET_HISTORY = T::GET_HISTORY,
            ...
            GET_PARAM_WORK_BRANCH = T::GET_PARAM_WORK_BRANCH,
            ASK_USER = T::ASK_USER,
            ASK_PLANNER = T::ASK_PLANNER,
            REPORT_ERROR = T::REPORT_ERROR,
            REPORT_RESULTS = T::REPORT_RESULTS,
        )
    }

    fn reviewer_instructions(&self, _tools: &ReviewerToolNames) -> String {
        use ReviewerToolNames as T;
        format!(
            r#"# Reviewer Agent
...
"#,
            ...
            GET_PARAM_WORK_BRANCH = T::GET_PARAM_WORK_BRANCH,
            REPORT_ERROR = T::REPORT_ERROR,
            REVIEW_ACCEPT = T::REVIEW_ACCEPT,
            REVIEW_REJECT = T::REVIEW_REJECT,
        )
    }

    fn tester_instructions(&self, _tools: &TesterToolNames) -> String {
        use TesterToolNames as T;
        format!(
            r#"# Tester Agent
...
"#,
            ...
            GET_PARAM_WORK_BRANCH = T::GET_PARAM_WORK_BRANCH,
            REPORT_ERROR = T::REPORT_ERROR,
            TEST_ACCEPT = T::TEST_ACCEPT,
            TEST_REJECT = T::TEST_REJECT,
        )
    }

    fn merger_instructions(&self, _tools: &MergerToolNames) -> String {
        use MergerToolNames as T;
        let branch_isolation = branch_isolation_instruction();
        format!(
            r#"# Merger Agent
