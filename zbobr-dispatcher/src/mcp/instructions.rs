...
2. **Inspect all changes made in this task**: Call `{GET_PARAM_DESTINATION_BRANCH}` to get the base branch name. Then use `git diff <destination_branch>...HEAD` (three dots) to see ALL changes introduced by this task relative to the base branch. Do NOT use `git show HEAD` or `git diff HEAD~1` — those only show the latest commit and miss earlier work. Do NOT checkout the base branch (it may conflict with worktree setup) — use only `git diff` with the remote ref (e.g. `origin/<destination_branch>...HEAD`). You can also use `git log origin/<destination_branch>..HEAD` to see all commits in this branch.
