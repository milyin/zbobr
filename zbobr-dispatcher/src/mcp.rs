...
// bring submodules into the `mcp` namespace (files live under src/mcp/)
#[path = "mcp/common.rs"]
pub mod common;
#[path = "mcp/instructions.rs"]
pub mod instructions;
#[path = "mcp/traits.rs"]
pub mod traits;

// re-export tool name modules so callers can refer to them as `zbobr_dispatcher::mcp::<role>_tools`
pub use common::{merger_tools, planner_tools, preparator_tools, reviewer_tools, worker_tools};

#[path = "mcp/merger.rs"]
pub mod merger;
#[path = "mcp/planner.rs"]
pub mod planner;
#[path = "mcp/preparator.rs"]
pub mod preparator;
#[path = "mcp/reviewer.rs"]
pub mod reviewer;
#[path = "mcp/worker.rs"]
pub mod worker;
...
/// after opening it, which could race when tests run in parallel under
/// nextest.  Two processes might inspect the same candidate before either
/// started the server, resulting in duplicate ports.  By returning the bound
/// listener we guarantee the socket stays reserved.
...
async fn serve_mcp(base_port: u16, path: &str, router: axum::Router) -> anyhow::Result<u16> {
    // Bind the listener up front to avoid check-then-bind races when tests run
    // concurrently.
