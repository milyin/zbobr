...
/// backed by in-process filesystem backends — no CLI binary involved.
pub struct IntegrationTestEnv {
    pub base_path: PathBuf,
    pub workspaces_dir: PathBuf,
    pub name: &'static str,
    pub zbobr: ZbobrDispatcherDyn,
    /// Optional remote repository slug used by GitHub repo-backend tests.
    /// Always `None` for the filesystem backend.
