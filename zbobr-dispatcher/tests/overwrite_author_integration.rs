...
/// Test that git repository is properly discovered from nested directories.
#[test]
fn test_git_repo_discovery_from_nested_directory() {
    let temp_dir = TempDir::new().expect("Failed to create temp dir");
    let repo_path = temp_dir.path();

    // Initialize a git repository
    init_git_repo(repo_path);
    create_commit(repo_path, "Initial", "user", "user@example.com");
    ...
    assert_eq!(
        ...
        "Discovered repo root doesn't match expected"
    );
}
